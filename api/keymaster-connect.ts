...
import {
  beginGithubBootstrap,
  completeGithubManifest,
  GITHUB_BOOTSTRAP_PATHS,
  githubBootstrapErrorPage,
  verifyGithubInstallation,
} from '../src/keymaster/githubBootstrap.js';
...
function responseHeaders(contentType = 'application/json') {
  return {
    ...
    'content-security-policy': "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'self'; base-uri 'none'; frame-ancestors 'none'; form-action 'self' https://github.com",
  };
}
...
function sendHtml(res: ServerResponse, status: number, html: string) {
  res.writeHead(status, responseHeaders('text/html; charset=utf-8'));
  res.end(html);
}
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === 'GET') {
    if (path === GITHUB_BOOTSTRAP_PATHS.start || path === GITHUB_BOOTSTRAP_PATHS.callback || path === GITHUB_BOOTSTRAP_PATHS.setup) {
      if (!oidcToken) {
        sendHtml(res, 503, githubBootstrapErrorPage(new Error('workload_identity_unavailable')));
        return;
      }
      try {
        if (path === GITHUB_BOOTSTRAP_PATHS.start) {
          sendHtml(res, 200, await beginGithubBootstrap(url, SUPABASE_URL, oidcToken));
          return;
        }
        if (path === GITHUB_BOOTSTRAP_PATHS.callback) {
          const installUrl = await completeGithubManifest(url, SUPABASE_URL, oidcToken);
          res.writeHead(302, { ...responseHeaders('text/plain; charset=utf-8'), location: installUrl });
          res.end('Redirecting to GitHub installation.');
          return;
        }
        const verified = await verifyGithubInstallation(url, SUPABASE_URL, oidcToken);
        sendHtml(res, verified.status, verified.html);
        return;
      } catch (error) {
        sendHtml(res, 400, githubBootstrapErrorPage(error));
        return;
      }
    }

    sendHtml(res, 200, path === '/keymaster/import' ? importPage : connectPage);
    return;
  }
  ...
  if (path !== '/keymaster/connect' && path !== '/keymaster/import') {
