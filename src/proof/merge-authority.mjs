...
function normalizeChecks(checks) {
  ...
  return new Map(checks.map((check) => [check?.name, check?.status]));
}
...
export function evaluateMergeRequest(request, observedHead, { checkHead = true } = {}) {
  ...
  for (const required of request?.policy?.requiredChecks ?? []) {
    if (!checks.has(required)) reasons.push(`missing_check:${required}`);
    else if (checks.get(required) !== 'pass') reasons.push(`check_not_passed:${required}`);
  }
