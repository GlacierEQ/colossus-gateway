...
class TestGitHub:
    def test_health_ok(self):
        ...
        with patch(
            "httpx.get", return_value=_mock_response(200, {"login": "GlacierEQ"})
        ):
    ...
    def test_create_issue(self):
        ...
        with patch("httpx.post", return_value=_mock_response(201, resp_data)):
            result = GitHubConnector().create_issue(
                "GlacierEQ", "test-repo", "Test Issue"
            )
...
class TestNotion:
    ...
    def test_search(self):
        ...
        with patch(
            "httpx.post",
            return_value=_mock_response(200, {"results": [{"id": "page_1"}]}),
        ):
...
class TestPinecone:
    def test_health_ok(self):
        ...
        with patch(
            "httpx.get", return_value=_mock_response(200, {"name": "colossus-apex"})
        ):
    ...
    def test_query(self):
        ...
        with patch(
            "httpx.post", return_value=_mock_response(200, {"matches": matches})
        ):
...
class TestSentry:
    def test_health_ok(self):
        ...
        with patch(
            "httpx.get", return_value=_mock_response(200, {"slug": "glaciereq"})
        ):
